...
reset_sudo() {
  ...
  log "Resetting PostgreSQL password for user $USER using psql via su - postgres"

  log_exec psql -qAt -U $PGUSER -h $PGHOST -p $PGPORT -d postgres -c "ALTER USER $USER WITH PASSWORD '$NEWPASS';"
  RET=$?
  if [ $RET -ne 0 ]; then
    ...
    msgbox "Password for user $USER successfully reset"
    return 0
  fi
}
...
reset_psql() {
  ...
  log "Resetting PostgreSQL password for user $USER using psql directly"
  
  log_exec psql -q -h $PGHOST -U postgres -d postgres -p $PGPORT -c "ALTER USER $USER WITH PASSWORD '$NEWPASS';"
  RET=$?
  if [ $RET -ne 0 ]; then
