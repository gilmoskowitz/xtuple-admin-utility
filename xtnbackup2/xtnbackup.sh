...
setup() {
  local DIR
  for DIR in archive ini logs ; do
    if [ -d $DIR ] ; then
...
setcronjob() {
  ...
  if [ "$CRONHOUR" -eq "$CRONHOUR" ] 2>/dev/null ; then
    CRONHOUR=${CRONHOUR}
  else
    unset CRONHOUR
  fi

  if [ "$CRONMIN" -eq  "$CRONMIN" ] 2>/dev/null ; then
    CRONMIN=${CRONMIN}
  else
    unset CRONMIN
  fi
...
EOF
  ...
  for REMOVEME in $REMOVELIST ; do
    rm -rf $REMOVEME
    echo "$REMOVEME Deleted" >> $REMOVALLOG
  done

  for REMOVEMESQL in $REMOVELISTSQL ; do
    rm -rf $REMOVEMESQL
    echo "$REMOVEMESQL Deleted" >> $REMOVALLOG
  done
}
...
EOF
  ...
  # back up all databases _other_ than the ones listed i.e. postgres, template0, template1
